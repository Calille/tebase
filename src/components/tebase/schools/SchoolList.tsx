...
interface School {
  ...
  email: string;
  phone: string;
  type: "primary" | "secondary" | "college" | "special";
...
const SchoolList = ({
  ...
  const handleAddSchool = () => {
    ...
    if (onAddSchool) {
      ...
      setSchools([...schools, newSchoolWithId]);
    }
  ...
  const handleToggleFavorite = (schoolId: string) => {
    ...
    if (onUpdateSchool) {
      const schoolToUpdate = updatedSchools.find((c) => c.id === schoolId);
      if (schoolToUpdate) {
        onUpdateSchool(schoolToUpdate);
      }
    }
